...
import LocalStorageCryptoEngine from "../../src/security/LocalStorageCryptoEngine";
const tokenIo = require('../../src');
const Token = new tokenIo(TEST_ENV);

if (BROWSER) {
    describe('LocalStorage crypto engine', () => {
        beforeEach(() => {
        ...
        it('should create the localStorage crypto engine', () => {
            ...
            const engine = new LocalStorageCryptoEngine(memberId);
            assert.isOk(engine);
        ...
        it('should be able to create multiple engines', () => {
            ...
            const engine = new LocalStorageCryptoEngine(memberId);
            const pk1 = engine.generateKey('LOW');
            const engine2 = new LocalStorageCryptoEngine(memberId);
            const pk2 = engine2.generateKey('STANDARD');
            const engine3 = new LocalStorageCryptoEngine(memberId);
        ...
        it('should be able to log in with the active memberId', () => {
            ...
            const engine = new LocalStorageCryptoEngine(memberId);
            const engine2 = new LocalStorageCryptoEngine(memberId2);
            const engine3 = new LocalStorageCryptoEngine(memberId3);
            const pk1 = engine.generateKey('LOW');
            const pk2 = engine2.generateKey('LOW');
            const pk3 = engine3.generateKey('LOW');

            const engineNew = new LocalStorageCryptoEngine(
                LocalStorageCryptoEngine.getActiveMemberId());
        ...
        it('should fail to log in to an empty browser', () => {
            try {
                const engine = new LocalStorageCryptoEngine();
