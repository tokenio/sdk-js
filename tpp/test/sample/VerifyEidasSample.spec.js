...
describe('VerifyEidasSample test', () => {
    ...
    //     const tppAuthNumber = 'testAuthNumber';//Util.generateNonce();
    //     // generate keys
    //     const pki = forge.pki;
    //     const keyPair = pki.rsa.generateKeyPair(2048);
    //     const certificate = await TestUtil.generateEidasCertificate(keyPair, tppAuthNumber);
    //     console.log('AN: cert: ' + certificate);
    //
    //     // const member = await VerifyEidasSample.verifyEidas(certificate, tppAuthNumber, 'iron', keyPair.privateKey);
    //     // //assert.equal(response.status, 'SUCCESS'); // TODO: should I use KonsentusVerificationStatus here?
    //     // const aliases = await member.aliases(); // TODO(Anna): is it verified aliases?
    ...
    it('Should create and verify business member', async () => {
        ...
        'MIIC6zCCAdOgAwIBAgIGAW2OBKSAMA0GCSqGSIb3DQEBCwUAMCwxETAPBgNVBAMMCFRva2VuLmlvMRcwFQYDVQR' +
        'hDA50ZXN0QXV0aE51bWJlcjAeFw0xOTEwMDIxOTQ4NTFaFw0yMDEwMDIxOTQ4NTFaMCwxETAPBgNVBAMMCFRva2VuLmlvMRcwFQYDVQ' +
        'RhDA50ZXN0QXV0aE51bWJlcjCCASIwDQYJKoZIhvcNAQEBBQADggEPADCCAQoCggEBAM2eg5UT6RdDwVvRb1dvBJ2la7J158/+orF7E' +
        'Puz+J9f7OF9XIQ8CXDzt+iTRW+76gHBpK/3EEzIc7oNBZVUkoWYKvmT9EEVAtweXDFS259/MEjRtdISL3ISEk18vGHULoko6A1xjxG8E' +
        'GhBiifLbWTCITW7/Z/i46CslKltpdzJEX3dBqYlk+cSstUfch+3aytcuFMQJCVB6pFd+jO8JeJPwIT30Ff50uc/UM8t7Rkm4mejL4Jl1' +
        'VRmVfGq6pOEb06chPHy6Up6y4Jw6XeUpw4pMvkxJePNS7DEdXon8udQ5cD5QiRvBadvH2fsK5QZ1PkOXyVXqEzSz9NRI764FcMCAwEAA' +
        'aMTMBEwDwYDVR0TAQH/BAUwAwEB/zANBgkqhkiG9w0BAQsFAAOCAQEACzdex+RGLj+7YDwiYbJk40SpzDlk4ns2Bk5U/aMKPbsVsxu0q' +
        '4w8/envM8TB8Z8IrMW5axHv2rXaQ59TQvzMEx3NXSCzeS7ylOeNFokIgjbDVojVTpHwwXq74GavX1J1aMk0sopwmb1wF8WvpmbAH7zEB' +
        'yNpQn6qfftZ3iM9qVFE/o+sx9CbQ1KAHJUHOPjFMkwHDcgXByyW7j5+TCAHpk281s3GfziUMeM7BoOFrK+5F8ergolcQfOsruNfVHYWh' +
        'CHvMT4ICFydSJnoMTKT7g41KA3HWEUr0hqNLHTaypahUFGEVroRxMS5HlTGbDBl2qEf37t3zh636m0ndS5x9A==';

        const member = await VerifyEidasSample.verifyEidas(certificate, tppAuthNumber, 'iron', privateKey);
        //    assert.equal(response.status, 'SUCCESS'); // TODO: should I use KonsentusVerificationStatus here?
        const aliases = await member.aliases(); // TODO(Anna): is it verified aliases?
        assert.equal(aliases.length, 1);
        assert.equal(aliases[0].value, tppAuthNumber);
        assert.equal(aliases[0].type, 'EIDAS');
        // remove the alias from the member, so that it can be re-used next time the test is run
